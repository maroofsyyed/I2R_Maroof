...
class Actor(nn.Module):
    def __init__(self, input_dim, hidden_dim1, hidden_dim2, output_dim, learning_rate=1e-4):
        super(Actor, self).__init__()
        self.fc1 = ModulatedPlasticDense(input_dim, hidden_dim1)
        self.ln1 = nn.LayerNorm(hidden_dim1)
        self.fc2 = ModulatedPlasticDense(hidden_dim1, hidden_dim2)
        ...
        self.hebb1 = torch.zeros(input_dim + 1, hidden_dim1).to(device)
        self.hebb2 = torch.zeros(hidden_dim1 + 1, hidden_dim2).to(device)
...
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim1, hidden_dim2, learning_rate=1e-4):
        super(Critic, self).__init__()
        self.fc1 = ModulatedPlasticDense(state_dim, hidden_dim1)
        self.ln1 = nn.LayerNorm(hidden_dim1)
        self.fc2 = ModulatedPlasticDense(hidden_dim1 + action_dim, hidden_dim2)
        self.ln2 = nn.LayerNorm(hidden_dim2)
        self.fc3 = nn.Linear(hidden_dim2, hidden_dim2)
        self.fc4 = nn.Linear(hidden_dim2, 1)
        self.optimizer = optim.Adam(self.parameters(), lr=learning_rate)
        self.scheduler = optim.lr_scheduler.StepLR(self.optimizer, step_size=100, gamma=0.95)
        self.hebb1 = torch.zeros(state_dim + 1, hidden_dim1).to(device)
        self.hebb2 = torch.zeros(hidden_dim1 + action_dim + 1, hidden_dim2).to(device)
...
class DDPG:
    def __init__(self, state_dim, action_dim, hidden_dim1, hidden_dim2, buffer_size, batch_size, actor_lr, critic_lr, tau, gamma):
        self.actor = Actor(state_dim, hidden_dim1, hidden_dim2, action_dim, actor_lr).to(device)
        self.actor_target = Actor(state_dim, hidden_dim1, hidden_dim2, action_dim, actor_lr).to(device)
        self.critic = Critic(state_dim, action_dim, hidden_dim1, hidden_dim2, critic_lr).to(device)
        self.critic_target = Critic(state_dim, action_dim, hidden_dim1, hidden_dim2, critic_lr).to(device)
...
def train_ddpg(env_name, episodes=1000, max_t=1000, noise_scale=1.0):
    ...
    agent = DDPG(state_dim, action_dim, hidden_dim1, hidden_dim2, buffer_size, batch_size, actor_lr, critic_lr, tau, gamma)
