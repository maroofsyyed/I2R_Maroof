# -*- coding: utf-8 -*-
"""DDPG_hebb_lin_lin_antbullet.ipynb
...
class Actor(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, learning_rate=1e-4):
        ...
        self.hebb = torch.zeros(input_dim + 1, hidden_dim).to(device)

    def forward(self, state):
        x, _, self.hebb = self.fc1(state, self.hebb)
        x = torch.relu(self.fc2(x))
        x = self.tanh(self.fc3(x))
        return x

    def update_hebb(self, state):
        with torch.no_grad():
            x, _, self.hebb = self.fc1(state, self.hebb)
        return x
...
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim, learning_rate=1e-4):
        ...
        self.optimizer = optim.Adam(self.parameters(), lr=learning_rate)
        self.hebb = torch.zeros(state_dim + 1, hidden_dim).to(device)
...
if __name__ == "__main__":
    ...
    episodes = 1000
    max_steps = 1000
    scores, avg_scores = train_ddpg(env, agent, episodes, max_steps, noise_scale=0.2, noise_decay=0.99)
