# -*- coding: utf-8 -*-
"""PPO_hebb_lin_lin_antbullet.ipynb
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.actor_layer2 = nn.Sequential(
            nn.Linear(256, 256),
            nn.Tanh(),
...
if __name__ == "__main__":
    ...
    Transition = namedtuple('Transition', ('obs', 'actions', 'logprobs', 'rewards', 'dones', 'values'))
    Memory = namedtuple('Memory', ('observations', 'actions', 'logprobs', 'rewards', 'dones', 'values', 'advantages'))
    ...
    optimizer = optim.Adam(agent.parameters(), lr=args.learning_rate, eps=5e-4)

    # ALGO Logic: Rollout storage
    ...
    for update in range(1, num_updates + 1):
        ...
        hebb_actor = torch.zeros((args.num_envs, *agent.hebb_size)).to(device)
        hebb_critic = torch.zeros((args.num_envs, *agent.hebb_size)).to(device)
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, logprob, entropy, value, hebb_actor, hebb_critic = agent.get_action_and_value(next_obs, hebb=hebb_actor)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue, _, _ = agent.get_action_and_value(b_obs[mb_inds], b_actions[mb_inds], hebb_actor)
